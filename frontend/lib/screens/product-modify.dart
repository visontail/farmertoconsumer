...
class ProductModifyForm extends StatefulWidget {
  final int id;
  const ProductModifyForm({super.key, required this.id});

  @override
  _ProductModifyFormState createState() => _ProductModifyFormState();
}

class _ProductModifyFormState extends State<ProductModifyForm> {
  ...
  Future<void> _fetchProduct(int id) async {
    ...
    try {
      ...
      product = await productService.getProduct(id.toString());
            ...
            Column(
              ...
              children: [
                ...
                DropdownButtonFormField<QuantityUnit>(
                  value: quantityUnits.contains(selectedQuantityUnit) ? selectedQuantityUnit: null,
                    ...
                    onPressed: () async {
                      if (_formKey.currentState!.validate()) {
                        try {
                          ...
                          final productService = ProductService();
                          final updatedProduct = await productService.updateProduct(widget.id.toString(), updatedFields);
